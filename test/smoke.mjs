...
// ── Module imports — catches ReferenceError (computeFlags, readFile, etc.)
// ── These are the exact code paths that crashed in production.
...
// ── Real GGUF tests — only run if models exist on this machine
const hasModels = existsSync(MODEL_DIR);

describe("GGUF model pipeline", { skip: !hasModels }, () => {
  let models;
  ...
  it("computeFlags produces valid argv for each model", () => {
    for (const model of models) {
      ...
      // argv must include the model path
